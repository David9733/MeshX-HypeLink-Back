...
- **공지(Notice) API 구현**: 모놀리식 레이어드 구조에서 본사 공지 등록·조회·수정·삭제 API 설계 및 구현
- **프로모션(Promotion) API 구현**: 모놀리식 레이어드 구조에서 프로모션 등록·조회·관리 API 설계 및 구현
- **공지(Notice) MSA 고도화**: 모놀리식 레이어드 구조의 공지 서비스를 마이크로서비스 헥사고날 구조로 직접 분리 및 고도화

...
| 선택 | 이유 |
|------|------|
| **온프레미스 Kubernetes** | 물리 서버를 직접 클러스터로 구성하여 고가용성과 확장성 확보. 외부 클라우드 의존 없이 인프라 완전 제어 가능 |
| **Ansible** | K8s 클러스터 구축·설정 자동화로 일관된 환경 구성. 반복 가능한 인프라 프로비저닝으로 수작업 오류 제거 |
| **GitHub Webhook + Jenkins** | main 브랜치 Push 시 파이프라인 자동 트리거. 빌드마다 격리된 Pod 생성으로 빌드 간 환경 오염 없음. Gradle 캐시 볼륨 마운트로 반복 빌드 속도 개선 |

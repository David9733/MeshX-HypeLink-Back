...
- **프로모션 FK 설계 오류** : 초기 설계 시 프로모션이 옷·매장·카테고리 등 여러 엔티티를 바라보도록 구성되어 FK 관계가 뒤섞이고 데이터 구조가 불명확했습니다.
...
- **Swagger 문서 자동화** : API가 늘어날수록 Swagger 어노테이션을 일일이 작성하는 데 시간이 많이 소요됐습니다.
...
- **기획서 자동화** : 기존 기획서를 Word·PPT로 작성하면 버전 관리가 어렵고 형식 맞추는 데 시간이 소요됐습니다.

...
| **구글 시트** | 별도 설치 없이 브라우저에서 팀원 동시 편집 가능. 댓글·버전 이력으로 변경 사항 추적이 용이하여 WBS·요구사항 관리에 적합 |
| **Typst** | 마크업 코드 기반으로 기획서를 자동 생성하여 형식 일관성 확보. Word 대비 버전 관리가 가능하고 반복 작업 없이 동일한 레이아웃 유지 |
| **Figma** | 프로토타입과 아키텍처 다이어그램을 하나의 도구로 처리. 링크 공유만으로 파일 전달 없이 팀원·프론트엔드와 실시간 협업 가능 |
| **ERD Cloud** | 웹 기반으로 설치 없이 ERD 작성 및 팀원과 즉시 공유. MySQL DDL 자동 생성을 지원하여 DB 설계에서 스키마 작성까지 일관된 흐름 유지 |
| **미리캔버스** | 한국어 템플릿 다수 제공으로 발표 PPT 디자인 시간 절감. 브라우저 기반 협업으로 파일 버전 충돌 없이 팀원과 동시 편집 가능 |
...
- **기획서 자동화** : 기획서를 직접 작성하면 형식을 맞추는 데 시간이 많이 소요됐습니다.
  Typst(마크업 기반 문서 자동화 도구)에 작성한 코드를 입력하니 기획서가 자동으로 생성되어 문서 작업 시간을 크게 줄일 수 있었습니다.
  다양한 도구에 대한 정보를 꾸준히 접하고 관심을 가질수록 아는 만큼 일의 효율이 달라진다는 것을 체감했습니다.

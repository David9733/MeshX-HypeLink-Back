...
| 선택 | 이유 |
|------|------|
| **Typst** | 마크업 코드 기반으로 기획서를 자동 생성하여 형식 일관성 확보. Word 대비 버전 관리가 가능하고 반복 작업 없이 동일한 레이아웃 유지. 기획서를 직접 작성하면 형식을 맞추는 데 시간이 많이 소요됐으나, 코드를 입력하면 기획서가 자동으로 생성되어 문서 작업 시간을 크게 줄일 수 있었습니다 |

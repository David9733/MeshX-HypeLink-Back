...
- **프로모션 FK 설계 오류 — 쿠폰 연결로 재설계** : 초기 설계 시 프로모션이 옷·매장·카테고리 등 여러 엔티티를 바라보도록 구성되어 FK 관계가 뒤섞이고 데이터 구조가 불명확했습니다.
  프로모션의 핵심 목적이 쿠폰 적용임을 재확인하고, FK를 쿠폰 단일 엔티티로 정리해 관계를 명확히 재설계했습니다.
  이 경험으로 FK 설계 전 도메인 간 책임과 관계를 먼저 명확히 정의하는 것이 구조 복잡도를 크게 줄인다는 것을 체감했습니다.

- **페이징 처리 위치 — 프론트 → 백엔드 이관** : 초기에는 프론트엔드에서 전체 데이터를 받아 클라이언트 단에서 페이징을 처리했으나, 데이터가 늘어날수록 불필요한 데이터 전송과 렌더링 부하가 커졌습니다.
...
- **Gradle 빌드 캐시 누락 — PVC 볼륨 마운트** : Jenkins Pod는 빌드마다 새로 생성되어 Gradle 의존성을 매번 처음부터 다운로드했고, 빌드 시간이 불필요하게 길었습니다.
...
- **Swagger 문서화 자동화 — AI 활용** : API가 늘어날수록 Swagger 어노테이션을 일일이 작성하는 데 시간이 많이 소요됐습니다.
  AI 바이브코딩을 활용해 Controller 코드를 기반으로 `@Operation`, `@ApiResponse` 등 Swagger 어노테이션을 자동 생성하는 방식으로 작업 속도를 높였습니다.
  반복적인 문서 작업에 AI를 보조 도구로 활용하면 핵심 로직 구현에 더 집중할 수 있다는 것을 체감했습니다.

- **기획서 자동화 — Typst 도입** : 기존 기획서를 Word·PPT로 작성하면 버전 관리가 어렵고 형식 맞추는 데 시간이 소요됐습니다.
  Typst를 활용해 코드 기반으로 기획서를 작성하고, Git으로 버전을 관리하면서 일관된 문서 형식을 유지했습니다.
  문서도 코드처럼 버전 관리할 수 있으면 협업 효율과 이력 추적이 크게 개선된다는 것을 깨달았습니다.
